...
module.exports = function(RED) {
    function ListServicesNode(config) {
        ...
        this.on('input', function(msg) {
            ...
            // Function to request the Services List from AMTE 

            async function getServicesList(token) {
                try {
                    const response = await axios({
                        method: 'GET',
                        url: 'http://amte.dataports-project.eu/api/services',
                        headers: {
            ...
            getTokenKeycloack().then(token => {
                getServicesList(token).then(servicesList => {
                    console.log(servicesList)
                    msg.payload = JSON.stringify(servicesList)
                    this.send(msg)
                })    
            })
        });
    }
    RED.nodes.registerType("list-services",ListServicesNode);
}
